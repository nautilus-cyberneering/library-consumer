...
for (let userInfo of userInfos) {
  describe(userInfo.key, () => {
    describe('importKey', () => {
      it.skip('imports key (as armored string) to GnuPG', async () => {
        await gpg.importKey(userInfo.pgp).then(output => {
          expect(output).not.toEqual('');
        });
      });
      it.skip('imports key (as base64 string) to GnuPG', async () => {
    ...
    describe('getKeygrips', () => {
      it.skip('returns the keygrips', async () => {
    ...
    describe('presetPassphrase', () => {
      it.skip('presets passphrase', async () => {
        await gpg.importKey(userInfo.pgp);
        await gpg.configureAgent(gpg.agentConfig);
