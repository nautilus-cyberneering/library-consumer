...
describe('CommitOptions', () => {
  it('should always include --allow-empty option', () => {
    ...
    const commitOptions = new CommitOptions(commitAuthor, singingKeyId, noGpgSig);
  ...
  it('should generate the git commit command arguments', () => {
    ...
    expect(commitOptions.toString()).toBe('--allow-empty --author="A Committer <committer@example.com>" --gpg-sign=3F39AA1432CA6AD7 --no-gpg-sign');
  });
});
